...
async function insert(ctx, next) {
  const { name, category, mainImage, price, discount, unit, remark, storage, introImage, introduction, status } = ctx.request.body;
  await DB('goods')
    .returning('id')
    .insert({
      name: name,
...
async function list(ctx, next) {
  const { id } = ctx.query;
  //查询单个数据才给出详情，列表不给
  const result = id ? DB('goods').where(ctx.query) : DB('goods').where(ctx.query).select('id', 'name', 'category', 'mainImage', 'price', 'discount', 'unit', 'remark', 'storage', 'status');
...
async function update(ctx, next) {
  const { id, name, category, mainImage, price, discount, unit, remark, storage, introImage, introduction, status } = ctx.request.body;
  if (!id) {
    ctx.state = { code: httpCode.paramNullCode, data: 'id参数为空', stateCode: httpCode.paramNullCode };
    return next();
  }
